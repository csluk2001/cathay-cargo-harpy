...
const useUploadedCsvStore = create<UploadedSheetState>((set, get) => ({
  ...
  updateUploadedSheetData: (uploadedSheet: UploadedSheet, parsedData) => {
    ...
    if (!state.sheetNameArray.includes(uploadedSheet.sheetName)) {
      set((state) => ({
        ...state,
        sheetNameArray: [...state.sheetNameArray, uploadedSheet.sheetName],
  ...
  castSheetStandardRateDataToSheetRowData: () => {
    set((state) => {
      let standardRateRowDataArray: ParsedStandardRateRowData[] = [];
      let standardRateWithFormulaRowDataArray: ParsedStandardRateWithFormulaRowData[] =
        [];

      for (let i = 0; i < state.sheetNameArray.length; i++) {
        if (
          state.sheetNameArray[i].trim().toLowerCase() === "standard rate (rm)"
        ) {
          ...
          for (let j = 0; j < tempSheetData.realData.length; j++) {
            for (let k = 0; k < tempSheetData.bySolution.length; k++) {
              // Create a new instance of the object for each iteration
              let tempStandardRateRowData = {
                cellId: ExcelHandler.generateCellId(k + 5) + (j + 3),
                rowId:
                ...
                destination: tempSheetData.realData[j].byRoute[3],
                solution: tempSheetData.bySolution[k],
                weightBreak: tempSheetData.byWeightBreak[k],
                rate: tempSheetData.realData[j].byWeightBreak[k],
                effectiveDate:
                  tempSheetData.realData[j].byWeightBreak[
                    tempSheetData.realData[j].byWeightBreak.length - 1
                  ],
              };
              ...
              let tempStandardRateWithFormulaRowData = {
                cellId: ExcelHandler.generateCellId(k + 5) + (j + 3),
                rowId:
                  tempSheetData.realData[j].byRoute[0] +
                  "-" +
              ...
              // Store data
              standardRateRowDataArray.push(tempStandardRateRowData);
      ...
      return {
        ...
        standardRateWithFormulaRowData: standardRateWithFormulaRowDataArray,
      };
    });
  },
  ...
  castSheetStandardRateFormulaDataToSheetRowData: () => {
    set((state) => {
      let standardRateFormulaRowDataArray = [];

      let tempStandardRateWithFormulaRowDataArray =
      ...
      for (let i = 0; i < state.sheetNameArray.length; i++) {
        if (
          ...
          for (let j = 0; j < tempSheetData.realData.length; j++) {
            for (let k = 0; k < state.sheetData[i].bySolution.length; k++) {
              // Create a new instance of the object for each iteration
              let tempStandardRateFormulaRowData = {
                cellId: ExcelHandler.generateCellId(k + 5) + (j + 3),
                rowId:
                  tempSheetData.realData[j].byRoute[0] +
                  "-" +
                ...
                solution: tempSheetData.bySolution[k],
                weightBreak: tempSheetData.byWeightBreak[k],
                formula: tempSheetData.realData[j].byWeightBreak[k],
                effectiveDate:
                  tempSheetData.realData[j].byWeightBreak[
                    tempSheetData.realData[j].byWeightBreak.length - 1
                  ],
                ...
                (tempStandardRateWithFormulaRowData) => {
                  if (
                    tempStandardRateWithFormulaRowData.cellId ===
                      ExcelHandler.generateCellId(k + 5) + (j + 3) &&
      ...
      return {
        ...
        standardRateWithFormulaRowData: tempStandardRateWithFormulaRowDataArray,
      };
    });
  },
  ...
  castSheetRateRationaleDataToSheetRowData: () => {
    set((state) => {
      let rateRationaleRowDataArray = [];

      for (let i = 0; i < state.sheetNameArray.length; i++) {
        if (state.sheetNameArray[i].trim().toLowerCase() === "rate rationale") {
          const tempSheetData = state.sheetData[i];
          ...
          for (let j = 0; j < tempSheetData.realData.length; j++) {
            for (let k = 0; k < state.sheetData[i].bySolution.length; k++) {
              ...
              let tempStandardRateRowData = {
                ...
                solution: tempSheetData.bySolution[k],
                weightBreak: tempSheetData.byWeightBreak[k],
                factor: tempSheetData.realData[j].byWeightBreak[k],
                effectiveDate:
                  tempSheetData.realData[j].byWeightBreak[
                    tempSheetData.realData[j].byWeightBreak.length - 1
                  ],
              ...
              // Store data
              rateRationaleRowDataArray.push(tempStandardRateRowData);
              // Debug
              // console.log(tempStandardRateRowData);
            }
          }
