...
interface UploadedSheetState {
	...
	editFormula: (cellId: string, newFormula: string) => void;
	translateFormula: () => void;
}
...
const useUploadedCsvStore = create<UploadedSheetState>((set, get) => ({
							...
							tempStandardRateWithFormulaRowDataArray.forEach(
								(tempStandardRateWithFormulaRowData) => {
									if (
										...
										tempStandardRateWithFormulaRowData.translatedFormula =
											tempStandardRateFormulaRowData.formula;
									}
								}
							);
	...
	editFormula() {},
	translateFormula() {
		for (let i = 0; i < this.standardRateWithFormulaRowData.length; i++) {
			for (
				let j = 0;
				j < this.standardRateWithFormulaRowData.length;
				j++
			) {
				if (
					this.standardRateWithFormulaRowData[i].formula.includes(
						this.standardRateWithFormulaRowData[j].cellId
					)
				) {
					let tempString =
						this.standardRateWithFormulaRowData[i].formula;
					tempString = tempString.replace(
						this.standardRateWithFormulaRowData[j].cellId,
						this.standardRateWithFormulaRowData[j].rowId
					);
					const tempStandardRateFormulaRowData =
						this.standardRateWithFormulaRowData;
					tempStandardRateFormulaRowData[i].translatedFormula =
						tempString;
